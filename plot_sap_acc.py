import matplotlib.pyplot as plt
import numpy as np

# x = np.linspace(-2,8, num=301)
# y = np.sinc((x-2.21)*3)


def annot_max(x,y, ax=None):
    xmax = x[np.argmax(y)]
    ymax = y.max()
    text= "iters={:.3f}, acc={:.3f}".format(xmax, ymax)
    if not ax:
        ax=plt.gca()
    bbox_props = dict(boxstyle="square,pad=0.3", fc="w", ec="k", lw=0.72)
    arrowprops=dict(arrowstyle="->",connectionstyle="angle,angleA=0,angleB=60")
    kw = dict(xycoords='data',textcoords="axes fraction",
              arrowprops=arrowprops, bbox=bbox_props, ha="right", va="top")
    ax.annotate(text, xy=(xmax, ymax), xytext=(0.94,0.96), **kw)

if __name__ == '__main__':
   acc = []
   acc_tmp = []
   iter = []
   j = 0
   with open('/media/ee4012/Disk3/Eric/Co-DETR/DA_stuff/outputs/BASE_WA_Test61+/20240427_155101.log', 'r') as r:
      file =  r.readlines()
      print(file)
      i = 0
      for l in file:
         if "t_acc:" in l:
            i = i + 1
            acc_tmp.append(float(l[52:-1]))
            # print(acc)
            if i % 4 == 0:
               acc.append(float(sum(acc_tmp) / 4.0))
               iter.append(int(i / 4))
               acc_tmp = []
      # acc.append(float(sum(acc_tmp) / i))
      # iter.append(int(i))
      

   # with open('20231127_122448.log', 'r') as r:
   #    file =  r.readlines()
   #    print(file)
   #    i = 0
   #    for l in file:
   #       if "acc" in l:
   #          i = i + 1
   #          if (i % 3 == 0):
   #             j = j + 100
   #             acc.append(float(l[46:-1]))
   #             print(acc)
   #             iter.append(int(j))

   # with open("20231128_163922.log", "r") as r :
   #    file =  r.readlines()
   #    print(file)
   #    i = 0
   #    for l in file:
   #       if "acc" in l:
   #          i = i + 1
   #          if (i % 3 == 0):
   #             j = j + 100
   #             acc.append(float(l[46:-1]))
   #             print(acc)
   #             iter.append(int(j))
   iter = iter[:]
   acc = acc[:]
   fig, ax = plt.subplots()
   iter = np.array(iter)
   acc = np.array(acc)
   ax.plot(iter,acc)
   ax.legend(loc='upper left')
   # print(acc) 
   annot_max(iter,acc)
   plt.plot(iter,acc, label='Dcls+GRL')
   plt.ylabel('accuracy')
   plt.xlabel('iterations')
   plt.legend()
   plt.show()
   print("acc:", acc)



